...
export default function () {
  ...
  if (doSearch) {
    ...
    const res = http.get(`${BASE_URL}/products-search-fixed?q=${encodeURIComponent(q)}&limit=20`, jsonHeaders);
    check(res, { 'fixed search handled': (r) => [200, 429].includes(r.status) });
  } else {
    const payload = JSON.stringify({ user_id: (__VU % 50) + 1, quantity: 1 });
    const res = http.post(`${BASE_URL}/cart/${PRODUCT_ID}/add-fixed`, payload, jsonHeaders);
    check(res, { 'fixed cart handled': (r) => [201, 400, 429].includes(r.status) });
  }
